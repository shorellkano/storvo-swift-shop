...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!store_id || !callback_url) {
      return new Response(JSON.stringify({ error: "store_id and callback_url are required" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const reference = `pro_${store_id}_${Date.now()}`;

    // Initialize Paystack transaction (₦3,500 = 350000 kobo)
